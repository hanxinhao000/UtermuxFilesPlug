        ...
        mutableListOf<NavigationItem>().apply {
            ...
            for (storageVolume in storageVolumes) {
                ...
                if (treeUris.remove(treeUri)) {
                   // add(UtermuxTreeStorageVolumeRootItem(treeUri, storageVolume))
...
private class UtermuxTreeStorageVolumeRootItem(
        private val treeUri: DocumentTreeUri,
        private val storageVolume: StorageVolume
) : LocalRootItem(
        treeUri.value.createDocumentTreeRootPath(), "/data/data/com.termux/files",
        R.drawable.ic_launcher
) {
    override fun getTitle(context: Context): String = "访问UTermux文件夹"
...
private val defaultStandardDirectories: List<StandardDirectory>
    ...
    get() =
        DEFAULT_STANDARD_DIRECTORIES.mapNotNull {
            when (it.iconRes) {
                R.drawable.qq_icon_white_24dp, R.drawable.tim_icon_white_24dp,R.drawable.ic_launcher,
                R.drawable.wechat_icon_white_24dp -> {
                    for (relativePath in it.relativePath.split(relativePathSeparator)) {

                        Log.e("输出路径" , relativePath)
                        val path = if(relativePath == "/data/data/com.termux/files"){
                            relativePath
                        }else{
                            getExternalStorageDirectory(relativePath)
                        }
...
private val DEFAULT_STANDARD_DIRECTORIES = listOf(
    ...
    StandardDirectory(
        R.drawable.ic_launcher, R.string.navigation_standard_directory_dcim,
        "/data/data/com.termux/files", true
    ),
    StandardDirectory(
...
internal fun getExternalStorageDirectory(relativePath: String): String =
   ...
   if(relativePath == "/data/data/com.termux/files"){relativePath}else{ Environment.getExternalStoragePublicDirectory(relativePath).path}
